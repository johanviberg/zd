...
func TestGetCacheHit(t *testing.T) {
	...
	assert.Equal(t, result1.Ticket.ID, result2.Ticket.ID, "expected same ticket from cache")
}
...
func TestListCacheHit(t *testing.T) {
	...
	assert.Equal(t, len(page1.Tickets), len(page2.Tickets), "expected same results from cache")
}
...
func TestListAuditsCacheHit(t *testing.T) {
	...
	assert.Equal(t, len(result1.Audits), len(result2.Audits), "expected same audits from cache")
}
...
func TestSearchCacheHit(t *testing.T) {
	...
	assert.Equal(t, len(page1.Results), len(page2.Results), "expected same search results from cache")
}
...
func TestUpdateInvalidatesCache(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, "solved", result.Ticket.Status, "expected status solved after update")
...
func TestCreateInvalidatesListAndSearch(t *testing.T) {
	...
	assert.Equal(t, count1+1, page2.Count, "expected count %d after create, got %d", count1+1, page2.Count)
}
